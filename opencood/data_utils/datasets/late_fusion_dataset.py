...
class LateFusionDataset(basedataset.BaseDataset):
    ...
    def data_reformat(self, observations, gt_info):
        ...
        if self.train:
            reformat_data_dict = self.get_item_train(observations)
        else:
            reformat_data_dict = self.get_item_test(observations, gt_info)

        return reformat_data_dict

    def get_item_single_car(self, selected_cav_base, gt_info):
        """
    ...
    def get_item_test(self, base_data_dict, gt_info):
        processed_data_dict = OrderedDict()
        ego_id = -999
        ego_lidar_pose = []
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            selected_cav_processed = \
                self.get_item_single_car(selected_cav_base, gt_info)
